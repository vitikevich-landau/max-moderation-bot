...
- **ML-фильтр токсичности** (rubert-tiny-toxicity) — анализ смысла текста
- Фильтрация запрещённых слов и доменов
- Автоматическая настройка при добавлении в чат
- Подключаемые словари модерации через внешние файлы (`filters/`)
...
| Принуд. откл. фильтров | Глобальное отключение фильтров слов, ссылок или токсичности для всех чатов |

## ML-фильтр токсичности

При установке скрипт предложит включить ML-фильтр на базе модели **rubert-tiny-toxicity**. Он анализирует смысл текста и определяет:

| Категория | Порог по умолчанию |
|-----------|-------------------|
| Общая токсичность | 0.85 |
| Оскорбления | 0.60 |
| Нецензурная лексика | 0.50 |
| Угрозы | 0.40 |

Пороги настраиваются per-chat через меню бота (кнопки с шагом 0.05).

При включённом ML-фильтре фильтр слов переключается на **точное совпадение** (по границам слов), что устраняет ложные срабатывания на подстроках. При выключенном — работает как раньше (поиск подстроки).

**Требования:** ~512MB RAM, первая сборка ~5 минут (скачивание модели).

**Fail-open:** если API токсичности недоступен, сообщения проходят. Фильтр слов продолжает работать как fallback.

## Мониторинг (опционально)

При установке скрипт спросит, нужен ли мониторинг. Если выбрать **да**, дополнительно развернутся:

- **Grafana** — веб-панель с графиками (http://localhost:3000, логин: `admin` / `admin`)
- **Prometheus** — сбор метрик (http://localhost:9091)
- **Экспортёры** — метрики PostgreSQL и Redis

Мониторинг показывает: скорость обработки сообщений, задержки, количество удалённых сообщений, активные мьюты, нагрузку на кэш и воркеры.

Требует ~512 МБ дополнительной оперативной памяти.

### Добавить мониторинг к существующей установке

Если бот уже установлен без мониторинга, запустите скрипт установки заново — он предложит добавить мониторинг.

### Пароль Grafana

По умолчанию пароль `admin`. Изменить можно в файле `prod.env`:
...
Бот разворачивается в 3 контейнерах (+ 4 при мониторинге, + 2 при ML-фильтре):
...
- **Toxicity API** *(опционально)* — FastAPI сервис с моделью rubert-tiny-toxicity
- **Toxicity UI** *(опционально)* — веб-интерфейс для тестирования (http://127.0.0.1:3001)
...
| `TOXICITY_ENABLED` | `false` | Включить ML-фильтр токсичности |
| `TOXICITY_API_URL` | `http://toxicity-api:8000` | Адрес API токсичности |
| `TOXICITY_SCORE_THRESHOLD` | `0.85` | Порог общей токсичности (0.05–1.00) |
| `TOXICITY_INSULT_THRESHOLD` | `0.60` | Порог оскорблений |
| `TOXICITY_OBSCENITY_THRESHOLD` | `0.50` | Порог нецензурной лексики |
| `TOXICITY_THREAT_THRESHOLD` | `0.40` | Порог угроз |

## Фильтры по умолчанию

При добавлении бота в чат автоматически загружаются фильтры из внешних файлов, которые монтируются в контейнер:

| Файл | Описание |
|------|----------|
| [`filters/blocked_words.txt`](filters/blocked_words.txt) | Запрещённые слова (одно слово на строку) |
| [`filters/blocked_domains.txt`](filters/blocked_domains.txt) | Заблокированные домены (один домен на строку) |

По умолчанию файлы **пустые**. Заполните их перед установкой или добавьте слова позже через меню бота.

### Как настроить фильтры

**До установки** — отредактируйте файлы в папке `filters/`:
- Добавьте по одному слову/домену на каждую строку
- При привязке бота к чату эти слова загрузятся автоматически

**После установки** — через меню бота в личных сообщениях:
